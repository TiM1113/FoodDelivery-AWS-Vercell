...
function makeRequest(body: Record<string, unknown>): NextRequest {
  return new NextRequest("http://localhost:3000/api/admin/orders/update", {
    method: "POST",
...
describe("POST /api/admin/orders/update", () => {
  it("returns 401 when not authenticated", async () => {
    ...
    const res = await POST(makeRequest({ orderId: "o1", status: "Delivered" }));
  ...
  it("returns 400 for invalid JSON", async () => {
    ...
    const req = new NextRequest("http://localhost:3000/api/admin/orders/update", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: "{invalid",
    });
  ...
  it("proxies status update to backend for admin", async () => {
    mockGetToken.mockResolvedValue({ id: "admin1", role: "admin" });
    vi.spyOn(globalThis, "fetch").mockResolvedValue(
      Response.json({ success: true, message: "Status updated" }),
    );

    const body = { orderId: "order123", status: "Delivered" };
    ...
    expect(fetchCall[0]).toContain("/api/order/update");
    const forwardedBody = JSON.parse(fetchCall[1]?.body as string);
    expect(forwardedBody.orderId).toBe("order123");
    expect(forwardedBody.status).toBe("Delivered");
  });
});
